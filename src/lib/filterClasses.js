...
RegExpFilter.fromText = function(text)
{
  ...
  let collapse = null;
  let refreshgpt = null;           
  let runlocalscripts = null;
  let overridejqueryready = null;
  let options;
  ...
  if (match)
  {
    ...
    for (let option of options)
    {
      ...
      else if (option[0] == "~" && option.substr(1) in RegExpFilter.typeMap)
      {
        ...
        sitekeys = value;
      else if (option == "REFRESHGPT")    
        refreshgpt = true;
      else if (option == "RUNLOCALSCRIPTS")
        runlocalscripts = true;
      else if (option == "OVERRIDEJQUERYREADY")
        overridejqueryready = true;
      else
  ...
  try
  {
    if (blocking) {
      ...
      return new AdsidebarFilter(origText, text, contentType, refreshgpt, runlocalscripts, overridejqueryready);
    }
    else {
...
/**
 * Class for Adsidebar filters (used for specifying per site Adsidebar options)
 * @param {String} text see Filter()
 ...
 */
function AdsidebarFilter(text, regexpSource, contentType, refreshgpt, runlocalscripts, overridejqueryready)
{
  RegExpFilter.call(this, text, regexpSource, contentType);

  this.refreshgpt = refreshgpt;
  this.runlocalscripts = runlocalscripts;
  this.overridejqueryready = overridejqueryready;
}
exports.AdsidebarFilter = AdsidebarFilter;

AdsidebarFilter.prototype =
{
  ...
  refreshgpt : null,
  runlocalscripts : null,
  overridejqueryready : null
};
