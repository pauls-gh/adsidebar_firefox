...
let UI = exports.UI =
{
  ...
  initDone: function()
  {
    ...
    // adsidebar message handling
    callback = this.adsidebarDataResponse.bind(this);
    Utils.addChildMessageListener("AdblockPlus:AdsidebarDataResponse", callback);

    // pass adsidebar preferences to the child process
    this.adsidebarUpdateChildPrefs();
  ...
  firstRunActions: function(window)
  {
    ...
    let {abp_addonVersion} = require("info");
    let prevVersion = Prefs.currentVersion;
    if (prevVersion != abp_addonVersion)
    {
      Prefs.currentVersion = abp_addonVersion;
  ...
  addSubscription: function(/**Window*/ window, /**String*/ prevVersion)
  {
    ...
    /* adsidebar - do not enable acceptable ads by default for adsidebar as we want to find all ads.
    ...
    */
    ...
    {
        // add AdSidebar filter that supplements EasyList.txt
      let subscription = Subscription.fromURL(Prefs.subscriptions_adsidebarurl);
      if (subscription && !(subscription.url in FilterStorage.knownSubscriptions))
      {
        FilterStorage.addSubscription(subscription);
        if (subscription instanceof DownloadableSubscription && !subscription.lastDownload)
  ...
  fillIconMenu: function(/**Event*/ event, /**Window*/ window, /**String*/ prefix)
  {
    ...
    /* open/close adsidebar menu item */
    this.adsidebarDataRequest(window, data => {
      if (data) {
        if (!data.sidebarEnabled) {
          setDisabled(prefix + "openadsidebar", true)
          hideElement(prefix + "openadsidebar", false);
          hideElement(prefix + "closeadsidebar", true);
        } else {
          hideElement(prefix + "openadsidebar", data.sidebarDisplayed);
          hideElement(prefix + "closeadsidebar", !data.sidebarDisplayed);
        }
      }
    });
    
    // auto hide radio button
    switch (Prefs.adsidebar_autohide) {
      case 0:
        setChecked(prefix + "adsidebar-autohide-disable", true);
      break;
      case 10:
        setChecked(prefix + "adsidebar-autohide-10", true);
      break;
      case 5:
      default:
        setChecked(prefix + "adsidebar-autohide-5", true);
      break;
    }
    
    // ad scaling
    setChecked(prefix + "adsidebar-adscaling", Prefs.adsidebar_adScaling);
  ...
  // adsidebar specific UI

  adsidebar_maxResponseID :  0,
  adsidebar_callbacks : new Map(),

  /**
   * request adsidebar state
   */
  adsidebarDataRequest: function(/**Window*/ window, callback)
  {
    let {getBrowser} = require("appSupport");
    let browser = getBrowser(window);
    if ("selectedBrowser" in browser)
    ...
    let id = ++this.adsidebar_maxResponseID;
    this.adsidebar_callbacks.set(id, callback);

    messageManager.broadcastAsyncMessage("AdblockPlus:AdsidebarDataRequest", {
      outerWindowID,
      responseID: id
    });
  },
  ...
  /**
   * child message response  (AdsidebarDataResponse)
   */
  adsidebarDataResponse : function({responseID, data}) 
  {
    let callback = this.adsidebar_callbacks.get(responseID);
    this.adsidebar_callbacks.delete(responseID);
  ...
  /**
   * Toggles open/closed state of Ad sidebar
   */
  toggleAdsidebar: function(/**Window*/ window)
  {
    let {getBrowser} = require("appSupport");
    let browser = getBrowser(window);
    if ("selectedBrowser" in browser)
    ...
    messageManager.broadcastAsyncMessage("AdblockPlus:AdsidebarToggleSidebar", {
      outerWindowID
    });
   ...
   * auto hide - disable
   */
  adsidebarAutoHideDisable: function(/**Window*/ window)
  {
    Prefs.adsidebar_autohide = 0;
    this.adsidebarUpdateChildPrefs();
  },
   ...
   * auto hide - 5 seconds
   */
  adsidebarAutoHide5: function(/**Window*/ window)
  {
    Prefs.adsidebar_autohide = 5;
    this.adsidebarUpdateChildPrefs();
  },
   ...
   */
  adsidebarAutoHide10: function(/**Window*/ window)
  {
    Prefs.adsidebar_autohide = 10;
    this.adsidebarUpdateChildPrefs();
  },
   ...
   * Toggles ad scaling
   */
  toggleAdsidebarAdScaling: function(/**Window*/ window)
  {
  ...
  /**
   * pass adsidebar preferences to child process
   */
  adsidebarUpdateChildPrefs: function()
  {
    let messageManager = Cc["@mozilla.org/parentprocessmessagemanager;1"]
                           .getService(Ci.nsIMessageBroadcaster);
    ...
    messageManager.broadcastAsyncMessage("AdblockPlus:AdsidebarUpdateChildPrefs", {
      prefs
    });
...
let eventHandlers = [
  ...
  ["abp-command-adsidebar", "command", UI.toggleAdsidebar.bind(UI)],
  ["abp-command-adsidebar-autohide-disable", "command", UI.adsidebarAutoHideDisable.bind(UI)],
  ["abp-command-adsidebar-autohide-5", "command", UI.adsidebarAutoHide5.bind(UI)],
  ["abp-command-adsidebar-autohide-10", "command", UI.adsidebarAutoHide10.bind(UI)],
  ["abp-command-adsidebar-adscaling", "command", UI.toggleAdsidebarAdScaling.bind(UI)],
