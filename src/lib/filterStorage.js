...
let FilterStorage = exports.FilterStorage =
{
  ...
  get sourceFile()
  {
    ...
    if (!file)
    {
      ...
      try
      {
        file = IO.resolveFilePath(Services.prefs.getDefaultBranch("extensions.adsidebar.").getCharPref("data_directory"));
        if (file)
          file.append("patterns.ini");
      } catch(e) {}
    }
    ...
    if (!file)
      Cu.reportError("Adblock Plus: Failed to resolve filter file location from extensions.adsidebar.patternsfile preference");
