...
var Policy = exports.Policy =
{
  ...
  init: function()
  {
    ...
    // adsidebar    
    Utils.addChildMessageListener("AdblockPlus:MatchAdsidebarList", message => this.matchAdsidebarList(message));
    ...
    collapseStyle = Services.io.newURI("data:text/css," +
  ...
  /**
   * Checks whether an Adsidebar filter exists for the given URL.
   * This will provide per website configuration options for Adsidebar.
   * 
   ...
   */
  matchAdsidebarList: function({contentType, location, frames, isPrivate})
  {   
    
    let typeMask = RegExpFilter.typeMap.DOCUMENT;
    let docDomain = null;
    let thirdParty = null;
    let sitekey = null;
    let specificOnly = null;    
    
    let match = defaultMatcher.matchesAdsidebarList(location, typeMask,  docDomain, thirdParty, sitekey, specificOnly);

    function response(foundMatch, refreshgpt, runlocalscripts, overridejqueryready)
    {
      return {foundMatch, refreshgpt, runlocalscripts, overridejqueryready};
    }

    if (match) {
      return response(true, match.refreshgpt, match.runlocalscripts, match.overridejqueryready);
    } else {
