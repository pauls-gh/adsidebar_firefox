...
let {Filter, RegExpFilter, WhitelistFilter, AdsidebarFilter} = require("filterClasses");
...
function CombinedMatcher()
{
  ...
  this.adsidebarlist = new Matcher();
...
CombinedMatcher.prototype =
{
  ...
  add: function(filter)
  {
    ...
    if (filter instanceof AdsidebarFilter) {
      this.adsidebarlist.add(filter);
  ...
  {
    if (filter instanceof AdsidebarFilter) {
      this.adsidebarlist.remove(filter);
    } else if (filter instanceof WhitelistFilter)
  ...
  {
    if (filter instanceof AdsidebarFilter) {
      return this.adsidebarlist.findKeyword(filter);
    } else if (filter instanceof WhitelistFilter)
  ...
  {
    if (filter instanceof AdsidebarFilter) {
      return this.adsidebarlist.hasFilter(filter);
    } else if (filter instanceof WhitelistFilter)
  ...
  {
    if (filter instanceof AdsidebarFilter) {
      return this.adsidebarlist.getKeywordForFilter(filter);
    } else if (filter instanceof WhitelistFilter)
  ...
  isSlowFilter: function(/**RegExpFilter*/ filter) /**Boolean*/
  {
    ...
    let matcher = null;
    if (filter instanceof AdsidebarFilter) {
      matcher = this.adsidebarlist;
    } else if (filter instanceof WhitelistFilter)
      matcher = this.whitelist;
    else
      matcher = this.blacklist;   
   ...
   * @see Matcher#matchesAny
   * 
   * Searches adsidebar filters only
   */
  matchesAdsidebarList: function(location, typeMask, docDomain, thirdParty, sitekey, specificOnly)
  {
    return this.adsidebarlist.matchesAny(location, typeMask, docDomain, thirdParty, sitekey, specificOnly);
  }
}
